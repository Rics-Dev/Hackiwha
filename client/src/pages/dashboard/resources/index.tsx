...
import { PlusCircle, Search, Filter, Download, BookOpen, FileText, Video, Globe, Bookmark } from 'lucide-react'
import { useState } from 'react'
import { ResourceType } from '@/types/app'
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-muted/50">
                                        ...
                                        <th className="text-left p-3 font-medium hidden lg:table-cell">Subject</th>
                                        <th className="text-left p-3 font-medium hidden lg:table-cell">Language</th>
                                            ...
                                            <td className="p-3">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-muted-foreground hidden md:block line-clamp-1">{resource.description}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-3 hidden md:table-cell">{resource.type}</td>
                                            <td className="p-3 hidden lg:table-cell">{resource.subject}</td>
                                            <td className="p-3 hidden lg:table-cell">{resource.language}</td>
...
function getResourceIcon(type: string, size: number = 16) {
    switch (type) {
        case 'Textbook':
            return <BookOpen className={`h-${size/4} w-${size/4} text-primary/70`} />
        case 'Exam':
            return <FileText className={`h-${size/4} w-${size/4} text-primary/70`} />
        case 'Note':
            return <FileText className={`h-${size/4} w-${size/4} text-primary/70`} />
        case 'Video':
            return <Video className={`h-${size/4} w-${size/4} text-primary/70`} />
        case 'Article':
            return <FileText className={`h-${size/4} w-${size/4} text-primary/70`} />
        case 'Exercise':
            return <FileText className={`h-${size/4} w-${size/4} text-primary/70`} />
